            ...
            <div className="w-full max-w-2xl" onClick={e => e.stopPropagation()}>
                 <form onSubmit={handleSubmit} className="relative group">
                    <input
                        ...
                        autoFocus
                        className="w-full bg-white/10 backdrop-blur-xl border border-white/30 text-white text-lg md:text-xl placeholder-white/50 rounded-2xl px-4 py-3 md:px-6 md:py-5 focus:ring-4 focus:ring-pink-500/30 focus:outline-none focus:border-pink-400 transition-all shadow-2xl"
                    />
                    <button type="submit" className="absolute right-4 top-1/2 -translate-y-1/2 text-white/70 hover:text-white p-2 rounded-full hover:bg-white/10 transition-all">
                        <svg xmlns="http://www.w3.org/2000/svg" className="h-6 w-6 md:h-7 md:w-7" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z" /></svg>
                    </button>
                </form>
                        ...
                        <ul>
                            {suggestions.map(plan => (
                                <li key={plan.id} className="border-b border-white/5 last:border-none">
                                    ...
                                    >
                                        <div className="relative w-16 h-10 md:w-20 md:h-14 rounded-lg overflow-hidden flex-shrink-0">
                                        ...
                                        <div>
                                            <span className="text-white font-medium text-sm md:text-lg block group-hover:text-pink-300 transition-colors line-clamp-1">{plan.title}</span>
                                            <span className="text-white/50 text-xs md:text-sm">{plan.city}, {plan.country}</span>
                                        </div>
  ...
  return (
    <>
      <header className="fixed top-0 left-0 right-0 z-50">
        <nav className="m-2 md:m-4 bg-black/10 backdrop-blur-lg border border-white/20 rounded-2xl shadow-lg">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div className="flex items-center justify-between h-14 md:h-20">
              <div className="flex items-center">
                <button onClick={() => setActiveSection(Section.Inicio)} className="flex-shrink-0">
                  <img src={logoUrl} alt="Planifica Tu Sueño Logo" className="h-10 md:h-20 w-auto" />
                </button>
              </div>
              ...
              <div className="-mr-2 flex md:hidden items-center">
                ...
                <button
                  ...
                  className="ml-1 inline-flex items-center justify-center p-2 rounded-md text-white/80 hover:text-white hover:bg-white/20 focus:outline-none"
                >
          ...
          {isMenuOpen && (
            <div className="md:hidden bg-black/80 rounded-b-2xl backdrop-blur-xl">
