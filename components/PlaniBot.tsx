...
const PlaniBot: React.FC<PlaniBotProps> = ({ planibotAvatarUrl, contactInfo, socialLinks, travelPlans, faqs, onOpenPlan }) => {
  ...
  const addModelMessageWithTyping = (text: string, extra: Partial<ChatMessage> = {}) => {
      ...
      const intervalId = window.setInterval(() => {
          ...
          if (chunk) {
              setMessages(prev => {
                  ...
                  if (newMessages.length > 0) newMessages[newMessages.length - 1].text += chunk;
                  return newMessages;
              });
              scrollToBottom();
              i += chunkSize;
    ...
    if ('webkitSpeechRecognition' in window || 'SpeechRecognition' in window) {
        ...
        recognitionRef.current.onresult = (event: any) => {
            ...
            if (transcript.trim().length > 0) {
                silenceTimerRef.current = window.setTimeout(() => handleSendMessage(null, transcript), 4000); 
            }
        };
  ...
  useEffect(() => {
      if (messages.length > 0) localStorage.setItem('planiBotHistory', JSON.stringify(messages));
  }, [messages]);
  ...
  return (
    <>
        ...
        <div className="fixed bottom-24 right-5 z-40 w-[calc(100vw-40px)] sm:w-96 h-[75vh] max-h-[650px] flex flex-col rounded-3xl shadow-2xl animate-fade-in-up overflow-hidden glass-container border border-white/20">
          ...
          <div className="flex-shrink-0 flex items-center justify-between p-4 bg-white/10 backdrop-blur-lg border-b border-white/10">
            ...
            <div className="flex items-center gap-1">
                 ...
                 <button onClick={() => setIsVoiceEnabled(!isVoiceEnabled)} className="p-2 rounded-full hover:bg-white/10 text-white/80 transition-colors" title={isVoiceEnabled ? "Silenciar Voz" : "Activar Voz"}>
                     {isVoiceEnabled ? <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" viewBox="0 0 20 20" fill="currentColor"><path fillRule="evenodd" d="M9.383 3.076A1 1 0 0110 4v12a1 1 0 01-1.707.707L4.586 13H2a1 1 0 01-1-1V8a1 1 0 011-1h2.586l3.707-3.707a1 1 0 011.09-.217zM14.657 2.929a1 1 0 011.414 0A9.972 9.972 0 0119 10a9.972 9.972 0 01-2.929 7.071 1 1 0 01-1.414-1.414A7.971 7.971 0 0117 10c0-2.21-.894-4.208-2.343-5.657a1 1 0 010-1.414zm-2.829 2.828a1 1 0 011.415 0A5.983 5.983 0 0115 10a5.984 5.984 0 01-1.757 4.243 1 1 0 01-1.415-1.415A3.984 3.984 0 0013 10a3.983 3.983 0 00-1.172-2.828 1 1 0 010-1.415z" clipRule="evenodd" /></svg> : <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" viewBox="0 0 20 20" fill="currentColor"><path fillRule="evenodd" d="M9.383 3.076A1 1 0 0110 4v12a1 1 0 01-1.707.707L4.586 13H2a1 1 0 01-1-1V8a1 1 0 011-1h2.586l3.707-3.707a1 1 0 011.09-.217zM12.293 7.293a1 1 0 011.414 0L15 8.586l1.293-1.293a1 1 0 111.414 1.414L16.414 10l1.293 1.293a1 1 0 01-1.414 1.414L15 11.414l-1.293 1.293a1 1 0 01-1.414-1.414L13.586 10l-1.293-1.293a1 1 0 010-1.414z" clipRule="evenodd" /></svg> }
                 </button>
           ...
           {isVideoVisible && (
               <div className={`w-full bg-black/50 relative flex-shrink-0 transition-all duration-500 ease-in-out ${isShortVideo ? 'h-96' : 'h-48'}`}>
                   <button onClick={() => setIsVideoVisible(false)} className="absolute top-2 right-2 z-30 bg-black/60 hover:bg-red-500/80 text-white rounded-full p-1 transition-colors backdrop-blur-sm">
                       <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" viewBox="0 0 20 20" fill="currentColor"><path fillRule="evenodd" d="M4.293 4.293a1 1 0 011.414 0L10 8.586l4.293-4.293a1 1 0 111.414 1.414L11.414 10l4.293 4.293a1 1 0 01-1.414 1.414L10 11.414l-4.293 4.293a1 1 0 01-1.414-1.414L8.586 10 4.293 5.707a1 1 0 010-1.414z" clipRule="evenodd" /></svg>
                   </button>
                   <iframe width="100%" height="100%" src={`https://www.youtube.com/embed/${currentVideoId}?autoplay=1&mute=0&controls=1&loop=0&rel=0`} title="PLANIFICA TU SUEÑO" frameBorder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" referrerPolicy="strict-origin-when-cross-origin" allowFullScreen></iframe>
             ...
             <div className="space-y-4 relative z-10">
              {messages.map((msg, index) => (
                <div key={index} className={`flex flex-col ${msg.role === 'user' ? 'items-end' : 'items-start'}`}>
                  <div className={`max-w-[85%] p-3 rounded-2xl text-sm shadow-md relative group ${msg.role === 'user' ? 'bg-green-600/80 text-white rounded-br-none backdrop-blur-sm border border-green-500/30' : 'bg-black/40 text-white rounded-bl-none backdrop-blur-md border border-white/10' }`}>
                  ...
                  {msg.whatsappSummaryLink && <div className="max-w-[85%] mt-1"><WhatsappSummaryButton link={msg.whatsappSummaryLink} /></div>}
                  {msg.image && <div className="max-w-[85%] mt-2"><img src={msg.image} alt="Bot" className="rounded-xl w-full h-auto shadow-lg border border-white/20 animate-fade-in-up" onError={(e) => (e.currentTarget.style.display = 'none')}/></div>}
                  {msg.showDatePicker && index === messages.length - 1 && <ChatDatePicker onDateSelect={(date) => handleSendMessage(null, date)} />}
                  {msg.recommendedPlans && msg.recommendedPlans.length > 0 && <div className="mt-3 w-full flex gap-3 overflow-x-auto pb-2 pl-1 snap-x no-scrollbar">{msg.recommendedPlans.map(plan => <PlanMiniCard key={plan.id} plan={plan} onClick={() => onOpenPlan(plan)} />)}</div>}
                </div>
              ))}
              {isLoading && <div className="flex items-start"><div className="bg-black/40 text-white rounded-2xl rounded-bl-none backdrop-blur-md border border-white/10 p-3 shadow-md"><div className="flex gap-1.5 items-center h-5"><span className="w-1.5 h-1.5 bg-white/60 rounded-full animate-bounce"></span><span className="w-1.5 h-1.5 bg-white/60 rounded-full animate-bounce delay-100"></span><span className="w-1.5 h-1.5 bg-white/60 rounded-full animate-bounce delay-200"></span></div></div></div>}
              <div ref={chatEndRef} />
            </div>
          </div>
          ...
          <div className="flex-shrink-0 p-3 bg-white/10 backdrop-blur-lg border-t border-white/10">
            <form onSubmit={(e) => handleSendMessage(e)} className="flex items-center gap-2">
                 <button type="button" onClick={toggleListening} className={`w-10 h-10 flex-shrink-0 rounded-full flex items-center justify-center transition-all duration-300 ${isListening ? 'bg-red-500 animate-pulse text-white' : 'bg-white/10 text-white hover:bg-white/20'}`}>
                    {isListening ? <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" viewBox="0 0 20 20" fill="currentColor"><path fillRule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zM8 7a1 1 0 00-1 1v4a1 1 0 001 1h4a1 1 0 001-1V8a1 1 0 00-1-1H8z" clipRule="evenodd" /></svg> : <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" viewBox="0 0 20 20" fill="currentColor"><path fillRule="evenodd" d="M7 4a3 3 0 016 0v4a3 3 0 11-6 0V4zm4 10.93A7.001 7.001 0 0017 8a1 1 0 10-2 0A5 5 0 015 8a1 1 0 00-2 0 7.001 7.001 0 006 6.93V17H6a1 1 0 100 2h8a1 1 0 100-2h-3v-2.07z" clipRule="evenodd" /></svg>}
                 </button>
                <input type="text" value={input} onChange={(e) => setInput(e.target.value)} placeholder={isListening ? "Escuchando..." : "Escribe aquí..."} className="flex-1 bg-black/20 border border-white/10 text-white placeholder-white/40 rounded-full focus:ring-2 focus:ring-green-400 focus:outline-none py-2.5 px-4 text-sm transition-all" disabled={isLoading} autoFocus/>
                <button type="submit" disabled={isLoading || !input.trim()} className="w-10 h-10 flex-shrink-0 flex items-center justify-center rounded-full bg-green-500 text-white shadow-lg hover:bg-green-600 disabled:opacity-50 disabled:bg-gray-500 transition-all transform active:scale-95">
      ...
      <style>{`
        .glass-container { background: rgba(20, 20, 20, 0.6); backdrop-filter: blur(20px); -webkit-backdrop-filter: blur(20px); }
        .whatsapp-bg { background-color: transparent; background-image: url("data:image/svg+xml,%3Csvg width='60' height='60' viewBox='0 0 60 60' xmlns='http://www.w3.org/2000/svg'%3E%3Cg fill='none' fill-rule='evenodd'%3E%3Cg fill='%23ffffff' fill-opacity='0.05'%3E%3Cpath d='M36 34v-4h-2v4h-4v2h4v4h2v-4h4v-2h-4zm0-30V0h-2v4h-4v2h4v4h2V6h4V4h-4zM6 34v-4H4v4H0v2h4v4h2v-4h4v-2H6zM6 4V0H4v4H0v2h4v4h2V6h4V4H6z'/%3E%3C/g%3E%3C/g%3E%3C/svg%3E"); }
        .markdown-body b, .markdown-body strong { font-weight: 700; color: #d1fae5; }
